...
func saveToken(path string, token *oauth2.Token) {
	...
	if err != nil {
		log.Fatalf("Unable to cache oauth token: " + err.Error())
	}
	defer f.Close()
...
func GetService() (*drive.Service, error) {
	b, err := ioutil.ReadFile("pkg/conf/credentials.json")
	if err != nil {
...
func GetFiles(service *drive.Service, folderName string) ([]*drive.File, error) {

	files := service.Files.List()
	req, err := files.Q("(name = '" + folderName + "')").
